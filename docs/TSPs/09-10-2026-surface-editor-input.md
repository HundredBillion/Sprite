...
**Composition (checks 3-6): verified by the accent chooser.** The dead-key route by Option could not be used because the project owner runs a tiling window manager that intercepts Option before Sprite sees it (so an earlier "no accent" result was the window manager, not the code). Instead, press-and-hold on a letter in a focused dock raised macOS's accent chooser, and it appeared anchored at the dock's cursor cell. That popover is generated only after the text input system queries the focused element's input handler for its marked range and candidate-window position, so its correct placement proves the per-Surface input handler is installed and consulted by macOS end to end. The final hop, the chosen accent arriving as a single `{"type":"input","text":T}`, is the unit-tested path (`a_committed_composition_is_text_without_a_key`, the preedit routing) and was not captured only because focus moved to another application before the choice was pressed. The C1 negative-space check that would fail if the composition wiring were wrong, "plain typing is not doubled", also passed.
