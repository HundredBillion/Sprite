...
- [ ] Reuse `scroll.0.borrow().base_handle` for wheel, thumb dragging, and precise saved offsets. Derive the top index and positive intra-row offset from `-base_handle.offset().y` divided by the configured row height. In GPUI 0.2.2, uniform_list does not populate Div child bounds, so the base handle logical_scroll_top() and the test-only convenience method cannot supply this anchor reliably. Clamp against row count and viewport bounds. On reveal use `scroll_to_item` with non-strict strategy; restore uses the exact pixel offset. Report changed anchor/count during normal paint without scheduling a new repaint just to send the event.
