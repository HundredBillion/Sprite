...
#
# The icon is rendered from ../Sprite.png here rather than kept as a committed
...
for required in "$BINARY" "$TERMINFO_SOURCE" packaging/Sprite.png packaging/macos/Info.plist; do
    if [ ! -f "$required" ]; then
...
# Every size Finder and the Dock ask for, from one source image. The @2x names
...
for size in 16 32 128 256 512; do
    double=$((size * 2))
    sips -s format png -z "$size" "$size" packaging/Sprite.png \
        --out "$set_dir/icon_${size}x${size}.png" >/dev/null
    sips -s format png -z "$double" "$double" packaging/Sprite.png \
